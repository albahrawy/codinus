...
import { isArray, joinAsPath } from "@codinus/js-extensions";
import { Observable, ReplaySubject, defer, map, of, switchMap, tap, throwError } from "rxjs";
import { ApiConfig, CODINUS_REST_API_CONFIG_PATH, IHttpUrlResolver } from "./types";

const absUrlRegex = /^[a-zA-Z]+:\/\//;
const relUrlRegex = /^~/;

@Injectable({ providedIn: 'root' })
export class CSHttpUrlResolver implements IHttpUrlResolver {

    private apiConfig$ = new ReplaySubject<ApiConfig>(1); // ReplaySubject with buffer size 1

    private http = inject(HttpClient);
    private configPath = inject(CODINUS_REST_API_CONFIG_PATH, { optional: true }) ?? 'assets/config/api.is';

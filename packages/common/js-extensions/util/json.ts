 ...
 */
export function jsonParse<T = unknown>(input: string | T, reviver?: (this: unknown, key: string, value: unknown) => unknown): T | null {
    try {
        if (isObject(input))
            return input;
 ...
 */
export function jsonFlatten(source: IGenericRecord, connector = '_') {
    const target: IGenericRecord = {};
    function _flatObject(srcObj: IGenericRecord, key: string): void {
 ...
 */
export function jsonForEach<TItem>(source: { [key: string]: TItem }, callback: (key: string, value: TItem) => void,
 ...
 */
export function jsonMap<T>(source: IRecord<T>, callback: (value: T, key: string) => unknown, ignoreNull = false) {
 ...
 */
export function jsonToArray<T, O>(source: IRecord<T>, callback: (value: T, key: string, index: number) => O, ignoreNull = false) {
    return Object.keys(source).reduce((ob, k, i) => {
        const v = source[k];
        const res = callback(v, k, i);
        if (res != null || !ignoreNull)
            ob.push(res);
        return ob;
    }, [] as O[]);
 ...
 */
export function jsonToKeyValueArray<TValue>(source: Nullable<IRecord<TValue>>, ignoreNull = true): ({ key: string; value: TValue; })[] {
...
export function jsonToXml(obj: IGenericRecord, rootKey = 'Data', placeInNode = false): string {
...
function createXmlElement(node: HTMLElement, obj: IGenericRecord, isNode: boolean): HTMLElement {

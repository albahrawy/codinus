...
import { AllowedLValTypes, IPosition, LValAncestor, ProgramSourceType } from "../types/basic";
import { ParseBindingListFlags, ParseFunctionFlag, ParseStatementFlag, RegExPattern } from "../types/constants";
import { Nullable, IAction } from "@codinus/types";
import {
    AnyExport,
    AnyFunction,
    AnyImport,
...
export abstract class ParserExpression<SH extends ScopeHandlerBase<S>, S extends Scope = ScopeType<SH>> extends ParserBase<SH, S> {
    ...
    protected override parseProgram(program: IProgram, end?: number, sourceType?: ProgramSourceType): IProgram {
        ...
        program.dependencies = [...this.state.dependencies];
    ...
    protected parseBlockBody(node: IBlockStatementLike, topLevel: boolean, end: number,
        allowDirectives?: boolean, callBack?: IAction<boolean>): void {
    ...
    protected parseBlockOrModuleBlockBody(children: Array<Nullable<AnyNode>>, topLevel: boolean, end: number,
        directives?: Array<IDirective>, callBack?: IAction<boolean>): void {
    ...
    protected parseBlock(allowDirectives = false, createNewLexicalScope = true, callBack?: IAction<boolean>) {
    ...
    protected parseMethod<T extends IObjectMethod | IClassMethod | IClassPrivateMethod>(node: T, isGenerator: boolean, isAsync: boolean,
        isConstructor: boolean, allowDirectSuper: boolean, type: T["type"], inClassScope = false): T {
    ...
    protected parseFunctionBodyAndFinish<T extends AnyTsFunction>(node: T, type: T["type"], isMethod = false): T {
        this.parseFunctionBody(node, false, isMethod);
        return this.finishNode(node, type);
    }

    protected parseFunctionBody(node: AnyTsFunction, allowExpression?: boolean | null, isMethod = false): void {
    ...
    protected parseStatementOrSloppyAnnexBFunctionDeclaration(allowLabeledFunction = false) {
    ...
    protected toAssignable(node: Nullable<ExpressionNode>, isLHS = false): void {
        if (!node)
            return;
    ...
    protected toAssignableList(exprList?: Array<Nullable<ExpressionNode>>, trailingCommaLoc?: IPosition, isLHS = false): void {
        if (!exprList)
            return;
    ...
    protected parseVar(node: IVariableDeclaration, isFor: boolean, kind: VariableKind, allowMissingInitializer = false) {
    ...
    protected parseVarStatement(node: IVariableDeclaration, kind: VariableKind, allowMissingInitializer = false) {
        this.next();
    ...
    protected getOwn<T extends object>(object: T, key: keyof T) {
    ...
    protected checkParams(node: AnyTsFunction, allowDuplicates: boolean,
        isArrowFunction?: boolean | null, strictModeChanged = true): void {

...
export abstract class ParserBase<SH extends ScopeHandlerBase<S>, S extends Scope = ScopeType<SH>> {
    ...
    protected sawUnambiguousESM = false;
    protected ambiguousScriptDifferentAst = false;
    ...
    protected collectDependencies(node: ExpressionNode): void {
        ...
        if (node?.type === 'Identifier' && !ReservedWordsHelper.isReservedDependency(node.name))
            this.state.dependencies.add(node.name);
    }
    ...
    protected semicolon(allowAsi = true): void {
    ...
    protected checkIdentifier(at: Identifier, bindingType: BindingTypes, strictModeChanged = false) {
    ...
    protected readInt(radix: number, len?: number, forceLen = false, allowNumSeparator: boolean | "bail" = true): Nullable<number> {
